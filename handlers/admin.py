...
# Глобальные переменные для хранения данных (будут в context.bot_data, но для обратной совместимости оставляем)
mailing_data: Dict[int, Dict] = {}
...
async def handle_admin_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str, user_id: int):
    ...
    elif data == "math_contest_confirm":
...
async def slot_animation(context: ContextTypes.DEFAULT_TYPE, message, user, prize_emoji: str, prize_name: str, prize_amount: int, user_id: int):
