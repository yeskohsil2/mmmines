...
from .common import check_ban, check_subscription, send_subscription_prompt, safe_answer, format_amount, parse_bet_amount, LAST_CLICK_TIME
...
async def handle_session_not_found(query):
    """Обработчик истёкшей игровой сессии"""
    try:
...
def generate_crash_multiplier() -> float:
    """Генерирует случайный множитель для игры Краш"""
    r = random.random() * 100
    if r < 50:
...
# ==================== MINES ====================
async def mines_cell_click(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, r: int, c: int):
    query = update.callback_query

    sessions = context.bot_data.setdefault('MINES_SESSIONS', {})
    last_click = context.bot_data.setdefault('LAST_CLICK_TIME', {})
    COOLDOWN_SECONDS = context.bot_data.get('COOLDOWN_SECONDS', 2)

    cooldown = check_cooldown(user_id, last_click, COOLDOWN_SECONDS)
    if cooldown > 0:
...
async def mines_take_win(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    query = update.callback_query
...
async def mines_cancel_game(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    query = update.callback_query
...
async def send_mines_board(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    sessions = context.bot_data.setdefault('MINES_SESSIONS', {})
    session = sessions.get(user_id)
    if not session:
        return
...
# ==================== GOLD ====================
async def gold_choice(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, side: str):
    query = update.callback_query

    sessions = context.bot_data.setdefault('GOLD_SESSIONS', {})
    last_click = context.bot_data.setdefault('LAST_CLICK_TIME', {})
    COOLDOWN_SECONDS = context.bot_data.get('COOLDOWN_SECONDS', 2)
...
async def gold_take_win(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    query = update.callback_query
...
async def send_gold_board(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    sessions = context.bot_data.setdefault('GOLD_SESSIONS', {})
    session = sessions.get(user_id)
    if not session:
        return
...
# ==================== PYRAMID ====================
async def pyramid_cell_click(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, level: int, door: int):
    query = update.callback_query

    sessions = context.bot_data.setdefault('PYRAMID_SESSIONS', {})
    last_click = context.bot_data.setdefault('LAST_CLICK_TIME', {})
    COOLDOWN_SECONDS = context.bot_data.get('COOLDOWN_SECONDS', 2)
...
async def pyramid_take_win(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    query = update.callback_query
...
async def pyramid_cancel_game(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    query = update.callback_query
...
async def send_pyramid_board(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    sessions = context.bot_data.setdefault('PYRAMID_SESSIONS', {})
    session = sessions.get(user_id)
    if not session:
        return
...
# ==================== TOWER ====================
async def tower_cell_click(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, level: int, col: int):
    query = update.callback_query

    sessions = context.bot_data.setdefault('TOWER_SESSIONS', {})
    last_click = context.bot_data.setdefault('LAST_CLICK_TIME', {})
    COOLDOWN_SECONDS = context.bot_data.get('COOLDOWN_SECONDS', 2)
...
async def tower_take_win(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    query = update.callback_query
...
async def tower_cancel_game(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    query = update.callback_query
...
async def send_tower_start(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    sessions = context.bot_data.setdefault('TOWER_SESSIONS', {})
    session = sessions.get(user_id)
    if not session:
        return None
...
async def update_tower_board(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    sessions = context.bot_data.setdefault('TOWER_SESSIONS', {})
    session = sessions.get(user_id)
    if not session:
        return
...
async def rr_bullets_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sessions = context.bot_data.setdefault('RR_SESSIONS', {})
    sessions[user_id] = {
        'game_id': game_id,
...
async def dice_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dice_cooldown = context.bot_data.setdefault('dice_cooldown', {})
...
async def start_dice_game(update: Update, context: ContextTypes.DEFAULT_TYPE, game_id: int, chat_id: int):
...
# ==================== KNB ====================
async def knb_choice_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, game_id: str, choice: str):
    query = update.callback_query
    user_id = query.from_user.id

    games = context.bot_data.setdefault('KHB_GAMES', {})
...
async def knb_pvp_choice_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, game_id: str, choice: str):
    query = update.callback_query
    user_id = query.from_user.id
...
async def knb_accept_duel(update: Update, context: ContextTypes.DEFAULT_TYPE, duel_id: str):
    query = update.callback_query
    user_id = query.from_user.id

    duels = context.bot_data.setdefault('KHB_DUELS', {})
...
async def knb_cancel_duel(update: Update, context: ContextTypes.DEFAULT_TYPE, duel_id: str):
    query = update.callback_query
    user_id = query.from_user.id
...
async def bowling_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mines_sessions = context.bot_data.setdefault('MINES_SESSIONS', {})
    gold_sessions = context.bot_data.setdefault('GOLD_SESSIONS', {})
    pyramid_sessions = context.bot_data.setdefault('PYRAMID_SESSIONS', {})
    bowling_sessions = context.bot_data.setdefault('BOWLING_SESSIONS', {})
    ...
    bet_amount = parse_bet_amount(args[1], None)
...
async def crash_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if target_multiplier < 1.01 or target_multiplier > 20.00:
            await update.message.reply_text("❌ Множитель должен быть от 1.01 до 20.00")
            return
...
async def send_dice_choice(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, user_name: str, bet_amount: int):
...
async def dice_choice_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, choice_type: str, value: str = None):
...
async def process_dice_result(context: ContextTypes.DEFAULT_TYPE, user_id: int, dice_msg_id: int, session: dict, dice_value: int):
    """Обработка результата игры в Кубик"""
    try:
...
async def dice_cancel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    """Отмена игры"""
    query = update.callback_query
...
async def process_darts_result(context: ContextTypes.DEFAULT_TYPE, user_id: int, darts_msg_id: int, dice_value: int, session: dict):
    """Обработка результата игры в Дартс"""
    try:
        await asyncio.sleep(4.5)
...
__all__ = [
    ...
    'handle_dice_game_callbacks',
    'cubic_command',
    'crash_command',
    'darts_command',
    'bowling_command',
    'coinflip_command',
    'update_task_progress_for_game'
]
